...
const updateScrollEffects = () => {
  ...
  if (!reduceMotion && hero) {
    ...
    parallaxLayers.forEach((layer) => {
      ...
      layer.style.translate =
        `0 ${heroProgress * strength * window.innerHeight}px`;
    });
  }
   ...
   * ============================================================
   *
   * Вертикальный скролл страницы управляет горизонтальным
   * перемещением карточек:
   *
   * 01 → 02 → 03 → 04
   *
   * У нас 4 карточки, поэтому трек имеет ширину 400%.
   * Для полного прохождения всех четырёх экранов нужно
   * переместить трек на 75% его собственной ширины.
   * ============================================================
   */
  ...
  if (!reduceMotion && processStory && processTrack) {
    ...
    const translateX = -75 * storyProgress;
  ...
  if (!reduceMotion && contact) {
    ...
    const offset = Math.round((visible - 0.5) * 54);
...
document
  .querySelectorAll('.service-grid, .case-list, .founder-grid')
  .forEach((group) => {
    [
...
if (finePointer && !reduceMotion) {
  ...
  document
    .querySelectorAll('.service-card, [data-tilt-card]')
    .forEach((card) => {
    ...
    .forEach((element) => {
      element.addEventListener(
        ...
        (event) => {
          ...
          const x =
            (event.clientX -
              (rect.left + rect.width / 2)) * 0.13;

          const y =
            (event.clientY -
              (rect.top + rect.height / 2)) * 0.13;
...
class DigitalTerrain {
  ...
  elevation(x, z, time) {
    ...
    const pointerLift =
      Math.exp(-pointerDistance) *
      (
        0.16 * this.pointer.active +
        0.12 * this.burstPower
      );
